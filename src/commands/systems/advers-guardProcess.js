  ...
  async execute(interaction, Lang) {
    try {
        ...
        if(type === `deletemmesage`) {
            await Server.findOneAndUpdate({ id: interaction.guild.id }, { AdversGuardProcess: "REMOVE:MESSAGE" });
            await interaction.reply({ content: `Değiştirildi.`, ephemeral: true })
        } else if(type === `deleteandmuteuser`) {
            await Server.findOneAndUpdate({ id: interaction.guild.id }, { AdversGuardProcess: "REMOVE:MESSAGE_TIMEOUT:USER" });
            await interaction.reply({ content: `Değiştirildi.`,  ephemeral: true })
        }else if(type === `dleteandbanuser`) {
            await Server.findOneAndUpdate({ id: interaction.guild.id }, { AdversGuardProcess: "REMOVE:MESSAGE_BAN:USER" });
            await interaction.reply({ content: `Değiştirildi.`,  ephemeral: true })
        }
    } catch (error) {
    }
