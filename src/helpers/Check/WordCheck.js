...
async function checkAdvers(word) {
    ...
    if (InviteGuardReg.test(word)){
        response["DiscordInvite"] = true
        response["Toxicity"] = AI.ToxicityPoints.Advers
    } else {
      response["DiscordInvite"] = false;
    }
    if (LinkGuardReg.test(word)) {
      response["Link"] = true;
      response["Toxicity"] = AI.ToxicityPoints.Advers;
    } else {
      response["Link"] = false;
...
async function BlacklistedWord(word, guildID) {
  const ServerData = await Server.findOne({ id: guildID });
  const BlacklistedWords = ServerData.BlacklistedWords;

   if (BlacklistedWords.some(Word => ` ${word.toLowerCase()} `.includes(` ${Word} `))) {
    return true;
  } else {
    return false;
  }
...
async function allCheck(word) {
  const swearResult = await checkSwear(word);
  const adversResults = await checkAdvers(word);

  const TopResults = {};

  const swearToxicity = swearResult.toxicity || 0;
  const adversToxicity = adversResults.Toxicity || 0;

  TopResults["Toxicity"] = swearToxicity + adversToxicity;
  TopResults["LinkEnabled"] = adversResults.Link;
  TopResults["InviteEnabled"] = adversResults.DiscordInvite;
  TopResults["SwearEnabled"] = swearResult.swear;
  return TopResults;
}

module.exports = { checkSwear, checkAdvers, allCheck, BlacklistedWord };